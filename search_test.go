...
func TestIPCache(t *testing.T) {
	...
	cache.Set("192.168.1.1", "FR")
	cache.Set("10.0.0.1", "DE")
	...
	// Test cache overflow
	cache.Set("172.16.0.1", "US")
	cache.Set("8.8.8.8", "US") // Should reset the cache
...
func TestFindCountryForIP(t *testing.T) {
	...
	for _, r := range ipRanges {
		...
		_, err := manager.UpdateIPRangeCountry(r.ipRange, start, end, r.country)
		if err != nil {
	...
	for _, tc := range testCases {
		country, err := locator.FindCountryForIP(tc.ip)
	...
	_, err := manager.UpdateIPRangeCountry("1.0.0.0-1.0.0.255", start, end, "IT")
	if err != nil {
		t.Fatalf("Error updating IP range: %v", err)
	}

	// The search should still return FR because the entry is cached
	country, err := locator.FindCountryForIP("1.0.0.123")
...
func TestGetAllRangesForCountry(t *testing.T) {
	...
	ranges, err = locator.GetAllRangesForCountry("IT")
	if err != nil {
